...
The adapter counts the levels of the open transaction. `begin()` increases the count. `commit()` and `rollback()` decrease it. `getTransactionLevel()` returns the count. `isUnderTransaction()` returns `true` when the PDO connection has an active transaction.

As of 5.21, `begin()`, `commit()` and `rollback()` compare the count with the PDO connection first. When the count is more than zero but the connection has no active transaction, the adapter sets the count to zero. This occurs:

- after a statement that commits implicitly, such as `CREATE TABLE` on MySQL
- after a reconnect
- after the transaction ends outside the adapter

After this reset, `commit()` and `rollback()` throw `Phalcon\Db\Exceptions\NoActiveTransaction`. The adapter does not send a `COMMIT`, `ROLLBACK` or savepoint statement to the server.
...
// MySQL commits the open transaction before it runs this statement
$connection->execute('CREATE TABLE `co_invoices_archive` LIKE `co_invoices`');
...
// 0
echo $connection->getTransactionLevel();
}
```

When the server rejects a transaction statement, the count stays correct:

- `begin()` restores the count when the transaction or the savepoint does not start, then throws the original exception again
- `commit()` restores the count when the commit or the release of the savepoint fails, then throws the original exception again
- `rollback()` keeps the count decreased when the rollback fails
...
Models hydrate spatial columns into these objects on read when `orm.cast_on_hydrate` is enabled. See [Spatial Column Hydration](/5.21/db-models/) in the models documentation.
...
[db-adapter-abstractadapter]: /5.21/api/phalcon_db/#dbadapterabstractadapter
[db-adapter-adapterinterface]: /5.21/api/phalcon_db/#dbadapteradapterinterface
[db-adapter-pdo-abstractpdo]: /5.21/api/phalcon_db/#dbadapterpdoabstractpdo
[db-adapter-pdo-mysql]: /5.21/api/phalcon_db/#dbadapterpdomysql
[db-adapter-pdo-postgresql]: /5.21/api/phalcon_db/#dbadapterpdopostgresql
[db-adapter-pdo-sqlite]: /5.21/api/phalcon_db/#dbadapterpdosqlite
[db-adapter-pdofactory]: /5.21/api/phalcon_db/#dbadapterpdofactory
[db-column]: /5.21/api/phalcon_db/#dbcolumn
[db-columninterface]: /5.21/api/phalcon_db/#dbcolumninterface
[db-dialect]: /5.21/api/phalcon_db/#dbdialect
[db-dialect-mysql]: /5.21/api/phalcon_db/#dbdialectmysql
[db-dialect-postgresql]: /5.21/api/phalcon_db/#dbdialectpostgresql
[db-dialect-sqlite]: /5.21/api/phalcon_db/#dbdialectsqlite
[db-dialectinterface]: /5.21/api/phalcon_db/#dbdialectinterface
[db-enum]: /5.21/api/phalcon_db/#dbenum
[db-exception]: /5.21/api/phalcon_db/#dbexception
[db-index]: /5.21/api/phalcon_db/#dbindex
[db-indexinterface]: /5.21/api/phalcon_db/#dbindexinterface
[db-phql]: /5.21/db-phql/
[db-profiler]: /5.21/api/phalcon_db/#dbprofiler
[db-profiler-item]: /5.21/api/phalcon_db/#dbprofileritem
[db-rawvalue]: /5.21/api/phalcon_db/#dbrawvalue
[db-reference]: /5.21/api/phalcon_db/#dbreference
[db-referenceinterface]: /5.21/api/phalcon_db/#dbreferenceinterface
[db-result-pdo]: /5.21/api/phalcon_db/#dbresultpdoresult
[db-resultinterface]: /5.21/api/phalcon_db/#dbresultinterface
[events]: /5.21/events/
[logger]: /5.21/logger/
...
Source: https://docs.phalcon.io/5.21/db-layer/index.mdx
