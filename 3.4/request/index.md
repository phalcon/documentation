...
[Phalcon\Http\Request](/3.4/api/phalcon_http/) allows you to access the values stored in the `$_REQUEST`, `$_GET` and `$_POST` arrays and sanitize or filter them with the [filter](/3.4/filter/) service, (by default [Phalcon\Filter](/3.4/api/phalcon_filter/)). The following examples offer the same behavior:
...
Source: https://docs.phalcon.io/3.4/request/index.mdx
