...
[Phalcon Config](/3.4/config/) ([Phalcon\Config](/3.4/api/phalcon_config/)) allows us to manipulate the file in an object-oriented way. In this example, we're using an ini file for configuration but Phalcon has [adapters](/3.4/config/) for other file types as well. The configuration file contains the following settings:
...
A request can use many services and registering each service individually can be a cumbersome task. For that reason, the framework provides a variant of [Phalcon\Di](/3.4/api/phalcon_di/) called [Phalcon\Di\FactoryDefault](/3.4/api/phalcon_di/) whose task is to register all services providing a full-stack framework.
...
Instead of using raw PHP as the previous tutorial, we started to use [Volt](/3.4/volt/). This is a built-in template engine inspired by Jinja_ providing a simpler and friendly syntax to create templates. It will not take too long before you become familiar with Volt.
...
For the sake of simplicity, we have used [sha1](https://php.net/manual/en/function.sha1.php) to store the password hashes in the database, however, this algorithm is not recommended in real applications, use [bcrypt](/3.4/security/) instead.
...
Now let's find out how the application accomplishes this. The first thing to know is that there is a component called [Dispatcher](/3.4/dispatcher/). It is informed about the route found by the [Routing](/3.4/routing/) component. Then, it is responsible for loading the appropriate controller and execute the corresponding action method.
...
The hook events always receive a first parameter that contains contextual information of the event produced (`$event`) and a second one that is the object that produced the event itself (`$dispatcher`). It is not mandatory that plugins extend the class [Phalcon\Mvc\User\Plugin](/3.4/api/phalcon_mvc_user/), but by doing this they gain easier access to the services available in the application.
...
The form is declared using an object-oriented scheme based on the elements provided by the [forms](/3.4/forms/) component. Every element follows almost the same structure:
...
With the help of [Phalcon\Mvc\Model\Criteria](/3.4/api/phalcon_mvc_model_criteria/), we can create the search conditions intelligently based on the data types and values sent from the form:
...
A session bag, is a special attribute in a controller that persists between requests using the session service. When accessed, this attribute injects a [Phalcon\Session\Bag](/3.4/api/phalcon_session/) instance that is independent in each controller.
...
This class extends the [Phalcon\Mvc\User\Component](/3.4/api/phalcon_mvc_user/). It is not imposed to extend a component with this class, but it helps to get access more quickly to the application services. Now, we are going to register our first user component in the services container:
...
Source: https://docs.phalcon.io/3.4/tutorial-invo/index.mdx
