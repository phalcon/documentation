...
PHQL queries can be created just by instantiating the class [Phalcon\Mvc\Model\Query](/3.4/api/phalcon_mvc_model_query/):
...
From a controller or a view, it's easy to create/execute them using an injected `models manager` ([Phalcon\Mvc\Model\Manager](/3.4/api/phalcon_mvc_model_manager/)):
...
### Result Types
Depending on the type of columns we query, the result type will vary. If you retrieve a single whole object, then the object returned is a [Phalcon\Mvc\Model\Resultset\Simple](/3.4/api/phalcon_mvc_model_resultset/). This kind of resultset is a set of complete model objects:
...
still a resultset of type [Phalcon\Mvc\Model\Resultset\Simple](/3.4/api/phalcon_mvc_model_resultset/). However, each element is a standard object that only contain the two columns that were requested.
...
The result in this case is an object [Phalcon\Mvc\Model\Resultset\Complex](/3.4/api/phalcon_mvc_model_resultset/). This allows access to both complete objects and scalars at once:
...
Source: https://docs.phalcon.io/3.4/db-phql/index.mdx
