...
[Phalcon\Mvc\Model](/3.4/api/phalcon_mvc_model/) is the base for all models in a Phalcon application. It provides database independence, basic
CRUD functionality, advanced finding capabilities, and the ability to relate models to one another, among other services. [Phalcon\Mvc\Model](/3.4/api/phalcon_mvc_model/) avoids the need of having to use SQL statements because it translates
methods dynamically to the respective database engine operations.

:::warning[NOTE]
Models are intended to work with the database on a high layer of abstraction. If you need to work with databases at a lower level check out the [Phalcon\Db](/3.4/api/phalcon_db/) component documentation.
:::

## Creating Models
A model is a class that extends from [Phalcon\Mvc\Model](/3.4/api/phalcon_mvc_model/). Its class name should be in camel case notation:
...
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| `conditions`  | Search conditions for the find operation. Is used to extract only those records that fulfill a specified criterion. By default [Phalcon\Mvc\Model](/3.4/api/phalcon_mvc_model/) assumes the first parameter are the conditions. | `'conditions' => "name LIKE 'steve%'"`                            |
...
| `for_update`  | With this option, [Phalcon\Mvc\Model](/3.4/api/phalcon_mvc_model/) reads the latest available data, setting exclusive locks on each row it reads                                                                                | `'for_update' => true`                                            |
| `shared_lock` | With this option, [Phalcon\Mvc\Model](/3.4/api/phalcon_mvc_model/) reads the latest available data, setting shared locks on each row it reads                                                                                   | `'shared_lock' => true`                                           |
...
All the queries are internally handled as [PHQL](/3.4/db-phql/) queries. PHQL is a high-level, object-oriented and SQL-like language. This language provide you more features to perform queries like joining other models, define groupings, add aggregations etc.
...
These objects are more powerful than standard arrays. One of the greatest features of the [Phalcon\Mvc\Model\Resultset](/3.4/api/phalcon_mvc_model_resultset/) is that at any time there is only one record in memory. This greatly helps in memory management especially when working with large amounts of data.
...
Note that resultsets can be serialized and stored in a cache backend. [Phalcon\Cache](/3.4/api/phalcon_cache/) can help with that task. However, serializing data causes [Phalcon\Mvc\Model](/3.4/api/phalcon_mvc_model/) to retrieve all the data from the database in an array, thus consuming more memory while this process takes place.
...
An array could be passed to `save` to avoid assign every column manually. [Phalcon\Mvc\Model](/3.4/api/phalcon_mvc_model/) will check if there are setters implemented for the columns passed in the array giving priority to them instead of assign directly the values of the attributes:
...
[Phalcon\Mvc\Model](/3.4/api/phalcon_mvc_model/) is able to recognize the identity column. Depending on the database system, those columns may be serial columns like in PostgreSQL or auto_increment columns in the case of MySQL.
...
:::warning[NOTE]
Never use a [Phalcon\Db\RawValue](/3.4/api/phalcon_db/) to assign external data (such as user input) or variable data. The value of these fields is ignored when binding parameters to the query. So it could be used to attack the application injecting SQL. 
:::
...
Using [Phalcon\Mvc\Model](/3.4/api/phalcon_mvc_model/) in a stand-alone mode can be demonstrated below:
...
Source: https://docs.phalcon.io/3.4/db-models/index.mdx
