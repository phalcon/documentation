...
Setting up your routes using the [Phalcon\Mvc\Micro](/3.4/api/phalcon_mvc_micro/) applications verb methods (`get`, `post`, etc.) is much easier than setting up a router object with relevant routes and then injecting it in the application.
...
Since our controllers extend the [Phalcon\Mvc\Controller](/3.4/api/phalcon_mvc_controller/), all the dependency injection services are available with their respective registration names. For example:
...
Lazy loading can be easily achieved when setting your handler in your [Phalcon\Mvc\Micro\Collection](/3.4/api/phalcon_mvc_micro/):
...
We need a [Phalcon\Events\Manager](/3.4/api/phalcon_events/) object. This can be a newly instantiated object or we can get the one that exists in our DI container (if you have used the `FactoryDefault` one).
...
:::warning[NOTE]
The above example uses the [Phalcon\Mvc\View\Simple](/3.4/api/phalcon_mvc_view/) component, which uses relative paths instead of controllers and actions. You can use the [Phalcon\Mvc\View](/3.4/api/phalcon_mvc_view/) component instead, but to do so you will need to change the parameters passed to `render()`
:::
...
Source: https://docs.phalcon.io/3.4/application-micro/index.mdx
