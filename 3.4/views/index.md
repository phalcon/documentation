...
You are not required to implement all of the files mentioned above. [Phalcon\Mvc\View](/3.4/api/phalcon_mvc_view/) will simply move to the next view level in the hierarchy of files. If all three view files are implemented, they will be processed as follows:
...
Note the lines where the method `$this->getContent()` was called. This method instructs [Phalcon\Mvc\View](/3.4/api/phalcon_mvc_view/) on where to inject the contents of the previous view executed in the hierarchy. For the example above, the output will be:
...
Automatic rendering must be disabled in [Phalcon\Mvc\Application](/3.4/api/phalcon_mvc_application/) (if needed):
...
This is different to [Phalcon\Mvc\View](/3.4/api/phalcon_mvc_view/) who's `render()` method uses controllers and actions as parameters:
...
[Phalcon\Mvc\View](/3.4/api/phalcon_mvc_view/) integrates with `Phalcon\Cache` to provide an easier way to cache output fragments. You could manually set the cache handler or set a global handler:
...
:::warning[NOTE]
The frontend must always be [Phalcon\Cache\Frontend\Output](/3.4/api/phalcon_cache/) and the service `viewCache` must be registered as always open (not shared) in the services container (DI). 
:::
...
A template engine adapter is a class that acts as bridge between [Phalcon\Mvc\View](/3.4/api/phalcon_mvc_view/) and the template engine itself. Usually it only needs two methods implemented: `__construct()` and `render()`. The first one receives the [Phalcon\Mvc\View](/3.4/api/phalcon_mvc_view/) instance that creates the engine adapter and the DI container used by the application.
...
The order that the template engines are defined with `Phalcon\Mvc\View::registerEngines()` defines the relevance of execution. If [Phalcon\Mvc\View](/3.4/api/phalcon_mvc_view/) finds two views with the same name but different extensions, it will only render the first one.
...
The following example shows how to write a jQuery [ajax request](https://api.jquery.com/jQuery.ajax/) using a URL with the framework conventions. The service `url` (usually [Phalcon\Mvc\Url](/3.4/api/phalcon_mvc_url/)) is injected in the view by accessing a property with the same name:
...
Source: https://docs.phalcon.io/3.4/views/index.mdx
