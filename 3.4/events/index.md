...
## Naming Convention
Phalcon events use namespaces to avoid naming collisions. Each component in Phalcon occupies a different event namespace and you are free to create your own as you see fit. Event names are formatted as `component:event`. For example, as [Phalcon\Db](/3.4/api/phalcon_db/) occupies the `db` namespace, its `afterQuery` event's full name is `db:afterQuery`.

When attaching event listeners to the events manager, you can use `component` to catch all events from that component (eg. `db` to catch all of the [Phalcon\Db](/3.4/api/phalcon_db/) events) or `component:event` to target a specific event (eg. `db:afterQuery`).

## Usage Example
In the following example, we will use the EventsManager to listen for the `afterQuery` event produced in a MySQL connection managed by [Phalcon\Db](/3.4/api/phalcon_db/):
...
Instead of using lambda functions, you can use event listener classes instead. Event listeners also allow you to listen to multiple events. In this example, we will implement the [Phalcon\Db\Profiler](/3.4/api/phalcon_db/) to detect the SQL statements that are taking longer to execute than expected:
...
By extending [Phalcon\Mvc\User\Plugin](/3.4/api/phalcon_mvc_user/), you can access services from the DI, just like you would in a controller:
...
The [Phalcon\Events\ManagerInterface](/3.4/api/phalcon_events/) interface must be implemented to create your own EventsManager replacing the one provided by Phalcon.
...
Source: https://docs.phalcon.io/3.4/events/index.mdx
