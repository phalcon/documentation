...
In this case, the `PostsController` will handle this request. There is no a special location to put controllers in an application, they could be loaded using [Phalcon\Loader](/3.4/api/phalcon_loader/), so you're free to organize your controllers as you need.
...
Additional URI parameters are defined as action parameters, so that they can be easily accessed using local variables. A controller can optionally extend [Phalcon\Mvc\Controller](/3.4/api/phalcon_mvc_controller/). By doing this, the controller can have easy access to
...
## Injecting Services
If a controller extends [Phalcon\Mvc\Controller](/3.4/api/phalcon_mvc_controller/) then it has easy access to the service container in application. For example, if we have registered a service like this:
...
Source: https://docs.phalcon.io/3.4/controllers/index.mdx
