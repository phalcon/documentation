...
The [INVO](/3.4/tutorial-invo/) tutorial shows how to take advantage of dispatching events implementing a security filter with [Acl](/3.4/acl/)
...
:::warning[NOTE]
Methods on event listeners accept an [Phalcon\Events\Event](/3.4/api/phalcon_events/) object as their first parameter - methods in controllers do not. 
:::
...
From 3.1.x onwards the dispatcher also comes with an option to handle this internally for all models passed into a controller action by using [Phalcon\Mvc\Model\Binder](/3.4/api/phalcon_mvc_model_binder/).
...
It also introduces a new interface [Phalcon\Mvc\Model\Binder\BindableInterface](/3.4/api/phalcon_mvc_model_binder/) which allows you to define the controllers associated models to allow models binding in base controllers.
...
In your PostsController you need to define which model the controller is associated with. This is done by implementing the [Phalcon\Mvc\Model\Binder\BindableInterface](/3.4/api/phalcon_mvc_model_binder/) which will add the `getModelName()` method from which you can return the model name. It can return string with just one model name or associative array where key is parameter name.
...
Using the [EventsManager](/3.4/events/) it's possible to insert a hook point before the dispatcher throws an exception when the controller/action combination wasn't found:
...
The [Phalcon\Mvc\DispatcherInterface](/3.4/api/phalcon_mvc_dispatcher/) interface must be implemented to create your own dispatcher replacing the one provided by Phalcon.

Source: https://docs.phalcon.io/3.4/dispatcher/index.mdx
