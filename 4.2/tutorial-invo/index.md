...
## Routing
INVO uses the standard route that is built-in with the [Router](/4.2/routing/) component. These routes match the following pattern: 
...
[Phalcon Config](/4.2/config/) allows us to manipulate the file in an object-oriented way. In this example, we are using an `ini` file for configuration. The [Phalcon\Config](/4.2/config/) object has additional adapter that load configuration files from different sources. The configuration file has the following settings:
...
## Request Handling
If we skip to the end of the file (`public/index.php`), the request is finally handled by [Phalcon\Mvc\Application](/4.2/application/), which initializes all the services necessary for the application to run.
...
In the first line of the code block above, the [Application](/4.2/application/) class constructor receives the variable `$container` as an argument.

Since Phalcon is highly decoupled, we need the container to be able to access registered services from it in different parts of the application. The component in question is [Phalcon\Di](/4.2/di/). It is a service container, that also performs dependency injection and service location, instantiating all components as they are needed by the application.
...
A request can use many services and registering each service individually can be a cumbersome task. For that reason, the framework provides a variant of [Phalcon\Di](/4.2/di/) called [Phalcon\Di\FactoryDefault](/4.2/di/#factory-default)`. This class has pre-registered services to suit a full stack MVC application.
...
Here, we return an instance of the MySQL connection adapter. We can also add extra functionality, such as adding a [Logger](/4.2/logger/), a [profiler](/4.2/db-models-events/#profiling-sql-statements) to measure query execution times or even change the adapter to a different RDBMS.
...
We are using [Volt](/4.2/volt/) as our template engine instead of PHP. This is a built-in template engine inspired by [Jinja][jinja] providing a simple and user friendly syntax to create templates. If you have worked with [Jinja][jinja] or [Twig][twig] in the past, you will see many similarities.
...
:::info[NOTE]
For more information about Di services, please check the [Dependency Injection](/4.2/di/) document.
:::
...
In order to accomplish this, we need to use the [Dispatcher](/4.2/dispatcher/) component. When the user requests a page or URL, the application first identifies the page requested using the [Route](/4.2/routing/) component. Once the route has been identified and matched to a valid controller and action, this information is delegated to the [Dispatcher](/4.2/dispatcher/) which then loads the controller and executes the action.
...
By creating an [Events Manager](/4.2/events/) and attaching specific code to the `dispatcher` events, we now have a lot more flexibility and can attach our code to the dispatch loop or operation. 

### Events
The [Events Manager](/4.2/events/) allows us to attach listeners to a particular type of event. The event type that we are attaching to is `dispatch`. The code below attaches listeners to the `beforeExecuteRoute` and `beforeException` events. We utilize these events to check for 404 pages and also perform allowed access checks in our application.
...
We now have the structure to start verifying the role in the current session. We can check if the user has access to use the [ACL](/4.2/acl/). If the user does not have access, we will redirect them to the home screen.
...
The form is declared using an object-oriented scheme based on the elements provided by the [Phalcon\Forms\Form](/4.2/forms/) component. Each element defined follows almost the same setup:
...
A session bag, (`persistent` property) is a special attribute in a controller that persists data between requests using the session service. When accessed, this attribute injects a [Phalcon\Session\Bag](/4.2/session/#persistent-data) instance that is independent in each controller.
...
```
The [paginator](/4.2/pagination/) object receives the results obtained by the search. We also set a limit (results per page) as well as the page number. Finally we call `paginate()` to get the appropriate chunk of the resultset back.
...
This class extends the [Phalcon\Di\Injectable][di-injectable]. It is not necessary to do so but extending this component allows us to access all the application services. We are going to register this user component in the services container:
...
[di-injectable]: /4.2/api/phalcon_di/#di-injectable
[mvc-model-criteria]: /4.2/api/phalcon_mvc/#mvc-model-criteria

Source: https://docs.phalcon.io/4.2/tutorial-invo/index.mdx
