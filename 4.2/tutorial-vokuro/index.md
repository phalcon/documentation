...
Phalcon needs to be installed. Head over to the [installation](/4.2/installation/) page if you need help with installing Phalcon. Note that Phalcon v4 requires the PSR extension to be installed and loaded **before** Phalcon. To install PSR you can check the [php-psr][psr] GitHub page. 
...
| Option               | Description                                                                                                                                                              |
|----------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|                                                                                                                                                                          
| `APP_CRYPT_SALT`     | Random and long string that is used by the [Phalcon\Crypt](/4.2/crypt/) component to produce passwords and any additional security features                                    |
...
The constructor of the class first creates a new DI container and store it in a local property. We are using the [Phalcon\Di\FactoryDefault](/4.2/di/) one, which has a lot of services already registered for us. 

We then create a new [Phalcon\Mvc\Application](/4.2/application/) and store it in a property also. We also store the root path because it is useful throughout the application.
...
### Acl
`Vokuro\Plugins\Acl\Acl` is a component that implements an [Access Control List][acl] for our application. The ACL controls which user has access to which resources. You can read more about ACL in our [dedicated page](/4.2/acl/).
...
In order to have validation for user supplied data, we are utilizing the [Phalcon\Forms\Form](/4.2/forms/) and [Phalcon\Validation\*](/4.2/validation/) classes. These classes allow us to create HTML elements and attach validators to them. The form is then passed to the view, where the actual HTML elements are rendered on the screen. 
...
Here we are checking the request that came from the user, if it is a `POST`. Now that it is, we need to use the form validators and check if we have any errors. The [Phalcon\Http\Request](/4.2/request/) object, allows us to get that data easily by using:
...
If everything is fine, we use again the [Phalcon\Http\Request](/4.2/request/) object to retrieve the submitted data but also sanitize them. The following example strips the tags from the submitted `name` string:
...
Note that we never store clear text passwords. Instead we use the [Phalcon\Security](/4.2/security/) component and call `hash` on it, to transform the supplied password to a one way hash and store that instead. This way, if someone compromises our database, at least they have no access to clear text passwords.
...
The core method, the setup if you like happens in the `initialize` method. That is the spot where we set all the [relationships](/4.2/db-models-relationships/) for the model. For the `Users` class we have several relationships defined. Why relationships you might ask? Phalcon offers an easy way to retrieve related data to a particular model. 
...
The `beforeValidationOnCreate` will fire every time we have a new record (`Create`), before any validations occur. We check if we have a defined password and if not, we will generate a random string, then hash that string using [Phalcon\Security](/4.2/security/) and storing it in the `password` property. We also set the flag to change the password.
...
[di-serviceproviderinterface]: /4.2/api/phalcon_di/#di-serviceproviderinterface
...
Source: https://docs.phalcon.io/4.2/tutorial-vokuro/index.mdx
