...
## Not-Found (404) 
If an [Events Manager](/4.2/events/) has been defined, you can use it to intercept exceptions that are thrown when the controller/action pair are not found.
...
## Events
[Phalcon\Mvc\Dispatcher][mvc-dispatcher] is able to send events to an [Manager](/4.2/events/) if it is present. Events are triggered using the type `dispatch`. Some events when returning boolean `false` could stop the active operation. The following events are supported:
...
The [INVO][invo] sample application, demonstrates how you can take advantage of dispatching events, implementing a security filter with [Acl](/4.2/acl/)
...
[mvc-dispatcher]: /4.2/api/phalcon_mvc/#mvc-dispatcher
[mvc-dispatcher-exception]: /4.2/api/phalcon_mvc/#mvc-dispatcher-exception
[mvc-dispatcherinterface]: /4.2/api/phalcon_mvc/#mvc-dispatcherinterface
...
Source: https://docs.phalcon.io/4.2/dispatcher/index.mdx
