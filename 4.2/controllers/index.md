...
## Routing
[Routing](/4.2/routing/) is further explained in the relevant document. However the default route is:

```bash
/:module/:controller/:action/:parameter1/:parameter2
```

You can find more information about modules in the [application](/4.2/application/) document. For an application that does not have any modules, the default routes are:
...
The above will call the `InvoicesController` and `listAction`. The parameters will be available through the [request](/4.2/request/) in the controller and action.

Controller classes can be in any folder in your application, so long as your autoloader knows where to look for them when called. [Phalcon\Loader](/4.2/loader/) has numerous options for registering directories, namespaces etc. to help with the discovery of the controllers.
...
## Dispatch Loop
The dispatch loop will be executed within the [Dispatcher](/4.2/dispatcher/) until there are no actions left to be executed. In the examples above we showed code in only one action, which will be executed with the appropriate request.

We can utilize the [Dispatcher](/4.2/dispatcher/) object to forward the request to a different module, controller or action, thus creating a more complex flow of operations in the dispatch loop. 
...
## Events
Controllers automatically act as listeners for [dispatcher](/4.2/dispatcher/) [events](/4.2/events/), implementing methods with those event names allowing you to implement hook points before/after the actions are executed:
...
## Request - Response
If you have already registered a [Request](/4.2/request/) and [Response](/4.2/response/) services to your DI container or have simply instantiated the [Phalcon\Di\FactoryDefault][di-factorydefault] one, you can access these objects as properties in your controller.
...
[mvc-controller-bindmodelinterface]: /4.2/api/phalcon_mvc/#mvc-controller-bindmodelinterface
[mvc-controllerinterface]: /4.2/api/phalcon_mvc/#mvc-controllerinterface
[di-factorydefault]: /4.2/api/phalcon_di/#di-factorydefault
[request]: /4.2/api/phalcon_http/#http-request
[response]: /4.2/api/phalcon_http/#http-response
[session-bag]: /4.2/api/phalcon_session/#session-bag

Source: https://docs.phalcon.io/4.2/controllers/index.mdx
