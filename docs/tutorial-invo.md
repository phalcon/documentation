...
## Routing
INVO uses the standard route that is built-in with the [Router](routing.md) component. These routes match the following pattern: `/:controller/:action/:params`. This means that the first part of a URI is the controller, the second the controller action and the rest are the parameters.
...
[Phalcon Config](config.md) ([Phalcon\Config](api/Phalcon_Config.md)) allows us to manipulate the file in an object-oriented way. In this example, we're using an ini file for configuration but Phalcon has [adapters](config.md) for other file types as well. The configuration file contains the following settings:
...
If we skip to the end of the file (`public/index.php`), the request is finally handled by [Phalcon\Mvc\Application](api/Phalcon_Mvc_Application.md) which initializes and executes all that is necessary to make the application run:
...
Instead of using raw PHP as the previous tutorial, we started to use [Volt](volt.md). This is a built-in template engine inspired by Jinja_ providing a simpler and friendly syntax to create templates. It will not take too long before you become familiar with Volt.
...
For the sake of simplicity, we have used [sha1](https://php.net/manual/en/function.sha1.php) to store the password hashes in the database, however, this algorithm is not recommended in real applications, use [bcrypt](security.md) instead.
...
Now let's find out how the application accomplishes this. The first thing to know is that there is a component called [Dispatcher](dispatcher.md). It is informed about the route found by the [Routing](routing.md) component. Then, it is responsible for loading the appropriate controller and execute the corresponding action method.
...
We now have total control over the Dispatcher used in the application. Many components in the framework trigger events that allow us to modify their internal flow of operation. As the Dependency Injector component acts as glue for components, a new component called [EventsManager](events.md) allows us to intercept the events produced by a component, routing the events to listeners.


### Events Management
The [EventsManager](events.md) allows us to attach listeners to a particular type of event. The type that interests us now is 'dispatch'. The following code filters all events produced by the Dispatcher:
...
The hook events always receive a first parameter that contains contextual information of the event produced (`$event`) and a second one that is the object that produced the event itself (`$dispatcher`). It is not mandatory that plugins extend the class [Phalcon\Mvc\User\Plugin](api/Phalcon_Mvc_User.md), but by doing this they gain easier access to the services available in the application.
...
The form is declared using an object-oriented scheme based on the elements provided by the [forms](forms.md) component. Every element follows almost the same structure:
...
## Performing a Search
The `search` action has two behaviors. When accessed via POST, it performs a search based on the data sent from the form but when accessed via GET it moves the current page in the paginator. To differentiate HTTP methods, we check it using the [Request](request.md) component:
...
With the help of [Phalcon\Mvc\Model\Criteria](api/Phalcon_Mvc_Model_Criteria.md), we can create the search conditions intelligently based on the data types and values sent from the form:
...
A session bag, is a special attribute in a controller that persists between requests using the session service. When accessed, this attribute injects a [Phalcon\Session\Bag](api/Phalcon_Session.md) instance that is independent in each controller.
...
This class extends the [Phalcon\Mvc\User\Component](api/Phalcon_Mvc_User.md). It is not imposed to extend a component with this class, but it helps to get access more quickly to the application services. Now, we are going to register our first user component in the services container:
