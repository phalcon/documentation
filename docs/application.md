...
If you do not wish to use [Phalcon\Mvc\Application](api/Phalcon_Mvc_Application.md), the code above can be changed as follows:
...
The following replacement of [Phalcon\Mvc\Application](api/Phalcon_Mvc_Application.md) lacks of a view component making it suitable for Rest APIs:
...
Although the above implementations are a lot more verbose than the code needed while using [Phalcon\Mvc\Application](api/Phalcon_Mvc_Application.md), offers an alternative in bootstrapping your application. Depending on your needs, you might want to have full control of what should be instantiated or not, or replace certain components with those of your own to extend the default functionality.
...
When [Phalcon\Mvc\Application](api/Phalcon_Mvc_Application.md) have modules registered, always is necessary that every matched route returns a valid module. Each registered module has an associated class offering functions to set the module itself up. Each module class definition must implement two methods: `registerAutoloaders()` and `registerServices()`, they will be called by [Phalcon\Mvc\Application](api/Phalcon_Mvc_Application.md) according to the module to be executed.


## Application Events
[Phalcon\Mvc\Application](api/Phalcon_Mvc_Application.md) is able to send events to the [EventsManager](events.md) (if it is present). Events are triggered using the type `application`. The following events are supported:
