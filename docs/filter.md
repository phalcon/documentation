...
You can access a [Phalcon\Filter](api/Phalcon_Filter.md) object from your controllers when accessing `GET` or `POST` input data (through the request object). The first parameter is the name of the variable to be obtained; the second is the filter to be applied on it.
...
## Filtering data
In addition to sanitizing, [Phalcon\Filter](api/Phalcon_Filter.md) also provides filtering by removing or modifying input data to the format we expect.
...
## Adding filters
You can add your own filters to [Phalcon\Filter](api/Phalcon_Filter.md). The filter function could be an anonymous function:
...
The [Phalcon\FilterInterface](api/Phalcon_Filter.md) interface must be implemented to create your own filtering service replacing the one provided by Phalcon.
