...
Note that this feature also works on the [Phalcon\Http\Request](api/phalcon_http.md#http-request) object, when calling methods to retrieve data from `GET` and `POST`, namely `getQuery()` and `getPost()`.
