...
You can also use `$_SERVER['REQUEST_URI']`. This requires a bit more work, since we need to utilize the [Router](routing.md) component to populate the `$_SERVER['REQUEST_URI']`. Our routes setup needs to change to:
...
[url]: api/phalcon_url.md#url
[url-exception]: api/phalcon_url.md#url-exception
[urlinterface]: api/phalcon_url.md#urlinterface
