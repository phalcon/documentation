...
In this case, the `PostsController` will handle this request. There is no a special location to put controllers in an application, they could be loaded using [Phalcon\Loader](api/Phalcon_Loader.md), so you're free to organize your controllers as you need.
...
Additional URI parameters are defined as action parameters, so that they can be easily accessed using local variables. A controller can optionally extend [Phalcon\Mvc\Controller](api/Phalcon_Mvc_Controller.md). By doing this, the controller can have easy access to
...
There is no limit on the `forwards` you can have in your application, so long as they do not result in circular references, at which point your application will halt. If there are no other actions to be dispatched by the dispatch loop, the dispatcher will automatically invoke the view layer of the MVC that is managed by [Phalcon\Mvc\View](api/Phalcon_Mvc_View.md).


## Initializing Controllers
[Phalcon\Mvc\Controller](api/Phalcon_Mvc_Controller.md) offers the `initialize()` method, which is executed first, before any action is executed on a controller. The use of the `__construct()` method is not recommended.
...
## Injecting Services
If a controller extends [Phalcon\Mvc\Controller](api/Phalcon_Mvc_Controller.md) then it has easy access to the service container in application. For example, if we have registered a service like this:
...
If you're using Phalcon as a full-stack framework, you can read the services provided [by default](di.md) in the framework.


## Request and Response
Assuming that the framework provides a set of pre-registered services. We explain how to interact with the HTTP environment. The `request` service contains an instance of [Phalcon\Http\Request](api/Phalcon_Http.md) and the `response` contains a [Phalcon\Http\Response](api/Phalcon_Http.md) representing what is going to be sent back to the client.
...
Learn more about the HTTP environment in their dedicated articles [request](request.md) and [response](response.md).


## Session Data
Sessions help us maintain persistent data between requests. You can access a [Phalcon\Session\Bag](api/Phalcon_Session.md) from any controller to encapsulate data that needs to be persistent:
...
Controllers automatically act as listeners for [dispatcher](dispatcher.md) events, implementing methods with those event names allow you to implement hook points before/after the actions are executed:
