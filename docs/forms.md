# Forms
[Phalcon\Forms\Form](api/Phalcon_Forms.md) is a component that helps with the creation and maintenance of forms in web applications.
...
Each element in the form can be rendered as required by the developer. Internally, [Phalcon\Tag](api/Phalcon_Tag.md) is used to produce the correct HTML for each element and you can pass additional HTML attributes as the second parameter of `render()`:
...
[Phalcon\Forms\Form](api/Phalcon_Forms.md) extends [Phalcon\Di\Injectable](api/Phalcon_Di.md) so you have access to the application services if needed:
...
!!! info "NOTE"
    ...
    Learn more about filtering in Phalcon by reading the [Filter documentation](filter.md) 
...
## Form Elements
Phalcon provides a set of built-in elements to use in your forms, all these elements are located in the [Phalcon\Forms\Element](api/Phalcon_Forms.md) namespace:

| Name                             | Description                                                   |
|----------------------------------|---------------------------------------------------------------|
| [Phalcon\Forms\Element\Text](api/Phalcon_Forms.md)     | Generate `INPUT[type=text]` elements                          |
| [Phalcon\Forms\Element\Password](api/Phalcon_Forms.md) | Generate `INPUT[type=password]` elements                      |
| [Phalcon\Forms\Element\Select](api/Phalcon_Forms.md)   | Generate `SELECT` tag (combo lists) elements based on choices |
| [Phalcon\Forms\Element\Check](api/Phalcon_Forms.md)    | Generate `INPUT[type=check]` elements                         |
| [Phalcon\Forms\Element\TextArea](api/Phalcon_Forms.md) | Generate `TEXTAREA` elements                                  |
| [Phalcon\Forms\Element\Hidden](api/Phalcon_Forms.md)   | Generate `INPUT[type=hidden]` elements                        |
| [Phalcon\Forms\Element\File](api/Phalcon_Forms.md)     | Generate `INPUT[type=file]` elements                          |
| [Phalcon\Forms\Element\Date](api/Phalcon_Forms.md)     | Generate `INPUT[type=date]` elements                          |
| [Phalcon\Forms\Element\Numeric](api/Phalcon_Forms.md)  | Generate `INPUT[type=number]` elements                        |
| [Phalcon\Forms\Element\Submit](api/Phalcon_Forms.md)   | Generate `INPUT[type=submit]` elements                        |
| [Phalcon\Forms\Element\Text](api/Phalcon_Forms.md)     | Generate `INPUT[type=text]` elements                          |
| [Phalcon\Forms\Element\TextArea](api/Phalcon_Forms.md) | Generate `TEXTAREA` elements                                  |
