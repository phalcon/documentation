# View Helpers (Tags)
Writing and maintaining HTML markup can quickly become a tedious task because of the naming conventions and numerous attributes that have to be taken into consideration. Phalcon deals with this complexity by offering the [Phalcon\Tag](api/Phalcon_Tag.md) component which in turn offers view helpers to generate HTML markup.

This component can be used in a plain HTML+PHP view or in a [Volt](volt.md) template.

!!! warning "NOTE"

    This guide is not intended to be a complete documentation of available helpers and their arguments. Please visit the [Phalcon\Tag](api/Phalcon_Tag.md) page in the API for a complete reference. 
...
Available document type constants in [Phalcon\Tag](api/Phalcon_Tag.md) namespace are:
...
Actually, all produced URLs are generated by the component [Phalcon\Mvc\Url](api/Phalcon_Mvc_Url.md). The same links can be generated with Volt:
...
### From the Request
A special feature that the [Phalcon\Tag](api/Phalcon_Tag.md) helpers have is that they keep the values of form helpers between requests. This way you can easily show validation messages without losing entered data.
...
[Phalcon\Tag](api/Phalcon_Tag.md) offers helpers to change dynamically the document title from the controller. The following example demonstrates just that:
...
[Phalcon\Tag](api/Phalcon_Tag.md) also provide helpers to generate tags such as script, link or img. They aid in quick and easy generation of the static resources of your application
...
## Tag Service
[Phalcon\Tag](api/Phalcon_Tag.md) is available via the [tag](tag.md) service, this means you can access it from any part of the application where the services container is located:
...
Then change the definition of the service [tag](tag.md):
...
You can also check out [Volt](volt.md) a faster template engine for PHP, where you can use a more developer friendly syntax for helpers provided by [Phalcon\Tag](api/Phalcon_Tag.md).
