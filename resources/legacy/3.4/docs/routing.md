...
## Defining Routes
[Phalcon\Mvc\Router](api/Phalcon_Mvc_Router.md) provides advanced routing capabilities. In MVC mode, you can define routes and map them to controllers/actions that you require. A route is defined as follows:
...
Since you can add many routes as you need using the `add()` method, the order in which routes are added indicate their relevance, latest routes added have more relevance than first added. Internally, all defined routes are traversed in reverse order until [Phalcon\Mvc\Router](api/Phalcon_Mvc_Router.md) finds the one that matches the given URI and processes it, while ignoring the rest.
...
Then, using for example the component [Phalcon\Mvc\Url](api/Phalcon_Mvc_Url.md) we can build routes from its name:
...
## Default Behavior
[Phalcon\Mvc\Router](api/Phalcon_Mvc_Router.md) has a default behavior that provides a very simple routing that always expects a URI that matches the following pattern: `/:controller/:action/:params`
...
## Annotations Router
This component provides a variant that's integrated with the [annotations](annotations.md) service. Using this strategy you can write the routes directly in the controllers instead of adding them in the service registration:
