...
`Phalcon\Encryption\Security\JWT` contains components to issue, parse, and validate JSON Web Tokens as described in [RFC 7519][rfc-7519].
...
[Phalcon\Encryption\Security\JWT\Token\Item][security-jwt-token-item] is used internally to store a payload as well as its encoded state. Such payload can be the claims data or the headers' data. By using this component, we can extract the necessary information for each Token.
...
[Phalcon\Encryption\Security\JWT\Token\Token][security-jwt-token-token] is the component responsible for storing and calculating the JWT token. It accepts the headers, claims ( as [Phalcon\Encryption\Security\JWT\Token\Item][security-jwt-token-item] objects), and signature objects in its constructor. The object is a value container: it cannot be cast to a string and does not serialize to JSON - use `getToken()` whenever the JWT string itself is needed. It exposes:
...
A Builder component ([Phalcon\Encryption\Security\JWT\Builder][security-jwt-builder]) is available, utilizing chained methods, and ready to be used to create JWT tokens. All you have to do is instantiate the Builder object, configure your token, and call `getToken()`. This will return a [Phalcon\Encryption\Security\JWT\Token\Token][security-jwt-token-token] object which contains all the necessary information for your token. When instantiating the builder component, you have to supply the signer class. In the example below we use the [Phalcon\Encryption\Security\JWT\Signer\Hmac][security-jwt-signer-hmac] signer.
...
Returns a [Phalcon\Encryption\Security\JWT\Token\Token][security-jwt-token-token] object - not the JWT string. Call `getToken()` on the returned object to obtain the string.
...
You can use the [Phalcon\Encryption\Security\JWT\Validator][security-jwt-validator] object to validate each claim by calling the `validate*` methods with the necessary parameters (taken from the [Phalcon\Encryption\Security\JWT\Token\Token][security-jwt-token-token]). The internal `errors` array in the [Phalcon\Encryption\Security\JWT\Validator][security-jwt-validator] will be populated accordingly, returning the results with the `getErrors()` method.
...
As an alternative, you can `verify()` and `validate()` your token using the relevant methods in the [Phalcon\Encryption\Security\JWT\Token\Token][security-jwt-token-token] object.
...
The JWT signer interface extends a canonical contract in the `Phalcon\Contracts\Encryption\Security\JWT\Signer` namespace. New code should type-hint the contract. The `SignerInterface` type remains as a deprecated alias and will be removed in a future major version.
...
[security-jwt-builder]: /5.20/api/phalcon_encryption/#encryptionsecurityjwtbuilder
[security-jwt-exceptions-invalidaudiencetype]: /5.20/api/phalcon_encryption/#encryptionsecurityjwtexceptionsinvalidaudiencetype
[security-jwt-exceptions-unsupportedalgorithmexception]: /5.20/api/phalcon_encryption/#encryptionsecurityjwtexceptionsunsupportedalgorithmexception
[security-jwt-exceptions-validatorexception]: /5.20/api/phalcon_encryption/#encryptionsecurityjwtexceptionsvalidatorexception
[security-jwt-signer-abstractsigner]: /5.20/api/phalcon_encryption/#encryptionsecurityjwtsignerabstractsigner
[security-jwt-signer-hmac]: /5.20/api/phalcon_encryption/#encryptionsecurityjwtsignerhmac
[security-jwt-signer-none]: /5.20/api/phalcon_encryption/#encryptionsecurityjwtsignernone
[security-jwt-signer-signerinterface]: /5.20/api/phalcon_encryption/#encryptionsecurityjwtsignersignerinterface
[security-jwt-token-abstractitem]: /5.20/api/phalcon_encryption/#encryptionsecurityjwttokenabstractitem
[security-jwt-token-enum]: /5.20/api/phalcon_encryption/#encryptionsecurityjwttokenenum
[security-jwt-token-item]: /5.20/api/phalcon_encryption/#encryptionsecurityjwttokenitem
[security-jwt-token-parser]: /5.20/api/phalcon_encryption/#encryptionsecurityjwttokenparser
[security-jwt-token-signature]: /5.20/api/phalcon_encryption/#encryptionsecurityjwttokensignature
[security-jwt-token-token]: /5.20/api/phalcon_encryption/#encryptionsecurityjwttokentoken
[security-jwt-validator]: /5.20/api/phalcon_encryption/#encryptionsecurityjwtvalidator

Source: https://docs.phalcon.io/5.20/encryption-security-jwt/index.mdx
