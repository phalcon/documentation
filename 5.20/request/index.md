...
:::warning[SECURITY]
`getHttpHost()` returns the `Host` header sent by the client, which is attacker-controlled. `setStrictHostCheck()` only validates that the host is well-formed; it does not confirm that the host belongs to your application. Before using the returned value to build absolute URLs, password-reset links, or cache keys, validate it against your own list of expected hostnames.
:::
...
[di-injectionawareinterface]: /5.20/api/phalcon_di/#diinitializationawareinterface
...
[http-request-exceotuib]: /5.20/api/phalcon_http/#httprequestexception
[http-request-file]: /5.20/api/phalcon_http/#httprequestfile
[http-request-fileinterface]: /5.20/api/phalcon_http/#httprequestfileinterface
[http-requestinterface]: /5.20/api/phalcon_http/#httprequestinterface
...
Source: https://docs.phalcon.io/5.20/request/index.mdx
