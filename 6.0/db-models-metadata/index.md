...
The first group of constants (`MODELS_ATTRIBUTES` through `MODELS_EMPTY_STRING_VALUES`) indexes the attribute metadata array. `MODELS_COLUMN_MAP` and `MODELS_REVERSE_COLUMN_MAP` index a separate column-map array. The two families count from `0` independently, so the same index value means different things depending on which array you hold. The metadata cache adapters persist these arrays, so the index layout is a stored format.
...
The same columns can be described with PHP attributes. `Phalcon\Annotations\Models\MetaData` holds one class for each annotation of the table above. The strategy reads the two forms and produces the same metadata.
...
The model of the example above, written with attributes:
...
Four parameter names are different, because a PHP parameter name cannot be snake_case under PSR-12, and `default` is a reserved word of Zephir, which builds the C extension that this library mirrors. The strategy reads the two spellings of each pair, so an existing docblock keeps its behavior, and the two projects keep one name for each concept.

| Docblock             | Attribute          |
|----------------------|--------------------|
| `skip_on_insert`     | `skipOnInsert`     |
| `skip_on_update`     | `skipOnUpdate`     |
| `allow_empty_string` | `allowEmptyString` |
...
Use one spelling for each concept in one model. The two spellings exist so that the reader can change without a change to the models, and not so that they can be mixed.

The two readers give different types for the same literal. The parser stores every docblock literal as a string, and PHP resolves an attribute argument to its own type. `@Column(default=1900)` gives the string `'1900'` to the `defaultValues` metadata index, and `#[Column(defaultValue: 1900)]` gives the integer `1900`. Code that compares a default value with `===` must account for this.
...
Any exceptions thrown in the `Phalcon\Mvc\Model\MetaData` namespace will be of type `Phalcon\Mvc\Model\Exception`. You can use this exception to selectively catch exceptions thrown only from this component.
...
[annotations]: /6.0/annotations/
[apcu]: https://www.php.net/manual/en/book.apcu.php
[cache-adapter-factory]: /6.0/cache/#adapter-factory
[memcached]: https://www.memcached.org
[mvc-model]: /6.0/api/phalcon_mvc/#mvcmodel
[mvc-model-metadata]: /6.0/api/phalcon_mvc/#mvcmodelmetadata
[mvc-model-metadata-apcu]: /6.0/api/phalcon_mvc/#mvcmodelmetadataapcu
[mvc-model-metadata-libmemcached]: /6.0/api/phalcon_mvc/#mvcmodelmetadatalibmemcached
[mvc-model-metadata-memory]: /6.0/api/phalcon_mvc/#mvcmodelmetadatamemory
[mvc-model-metadata-redis]: /6.0/api/phalcon_mvc/#mvcmodelmetadataredis
[mvc-model-metadata-strategy-annotations]: /6.0/api/phalcon_mvc/#mvcmodelmetadatastrategyannotations
[mvc-model-metadata-strategy-introspection]: /6.0/api/phalcon_mvc/#mvcmodelmetadatastrategyintrospection
[mvc-model-metadata-strategyinterface]: /6.0/api/phalcon_mvc/#mvcmodelmetadatastrategystrategyinterface
[mvc-model-metadata-stream]: /6.0/api/phalcon_mvc/#mvcmodelmetadatastream
[mvc-model-metadatainterface]: /6.0/api/phalcon_mvc/#mvcmodelmetadatainterface
[redis]: https://redis.io

Source: https://docs.phalcon.io/6.0/db-models-metadata/index.mdx
