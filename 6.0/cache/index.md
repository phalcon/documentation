...
Frequently used data or already processed/calculated data, can be stored in a cache storage for easier and faster retrieval. This reduces the overhead that comes with getting data from any storage container. Some examples that warrant the use of a cache are:
...
:::info[NOTE]
The adapters differ in counter atomicity and in how `getKeys()` is implemented - for example `Redis` enumerates keys with a non-blocking `SCAN`, while `Stream` walks the directory tree. See the [adapter capability matrix](/6.0/storage/#capability-matrix) in the Storage documentation for the per-adapter details.
:::
...
[cache-abstract-cache]: /6.0/api/phalcon_cache/#cacheabstractcache
[cache-adapter-adapterinterface]: /6.0/api/phalcon_cache/#cacheadapteradapterinterface
[cache-adapter-apcu]: /6.0/api/phalcon_cache/#cacheadapterapcu
[cache-adapter-libmemcached]: /6.0/api/phalcon_cache/#cacheadapterlibmemcached
[cache-adapter-memory]: /6.0/api/phalcon_cache/#cacheadaptermemory
[cache-adapter-redis]: /6.0/api/phalcon_cache/#cacheadapterredis
[cache-adapter-stream]: /6.0/api/phalcon_cache/#cacheadapterstream
[cache-adapterfactory]: /6.0/api/phalcon_cache/#cacheadapterfactory
[cache-cache]: /6.0/api/phalcon_cache/#cachecache
[cache-cachefactory]: /6.0/api/phalcon_cache/#cachecachefactory
[cache-exception-exception]: /6.0/api/phalcon_cache/#cacheexceptionexception
[cache-exception-invalidargumentexception]: /6.0/api/phalcon_cache/#cacheexceptioninvalidargumentexception
[config]: /6.0/config/
...
Source: https://docs.phalcon.io/6.0/cache/index.mdx
