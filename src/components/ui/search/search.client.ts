...
export interface SearchConfig {
  ...
  onNavigate?: () => void;
  /** Version of the current page; results are scoped to it by default. */
  version?: string;
  /** Checkbox that lifts the version scope. */
  allVersions?: HTMLInputElement | null;
}
...
export function initSearch(config: SearchConfig): SearchInstance {
  const { input, resultsContainer, emptyState, provider, onNavigate, version, allVersions } = config;
  ...
  async function runSearch(query: string): Promise<void> {
    ...
    try {
      const scoped = version && !allVersions?.checked;
      const results = await provider.search(query, {
        signal,
        ...(scoped ? { filters: { version } } : {}),
  ...
  allVersions?.addEventListener("change", handleScopeChange);
  ...
  return {
    ...
    destroy() {
      ...
      allVersions?.removeEventListener("change", handleScopeChange);
    },
  };
}
...
mount("[data-search-dialog]", (root) => {
  ...
  const search = initSearch({
    ...
    version: dialog.dataset.searchVersion,
    allVersions: dialog.querySelector<HTMLInputElement>("[data-search-all-versions]"),
  });
