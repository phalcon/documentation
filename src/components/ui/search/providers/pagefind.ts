...
export const provider: SearchProvider = {
  ...
  async search(query, opts?: { signal?: AbortSignal; filters?: Record<string, string> }) {
    if (!pagefind) await this.init?.();
    if (!pagefind) return [];

    // Version scoping from the dialog merges with the site-wide defaults
    // (the deprecated-version exclusion).
    const filters = { ...(defaultFilters ?? {}), ...(opts?.filters ?? {}) };
